import Link from "next/link"
import { notFound } from "next/navigation"
import { ArrowLeft } from "lucide-react"

import watchCategories, { type WatchCategory } from "@/lib/watchCategories"
import { getEpisodes } from "@/lib/episodes"

// Import the generateMetadata function
import { generateMetadata } from "./generateMetadata"

// Re-export the generateMetadata function
export { generateMetadata }

export default async function WatchCategoryPage({
  params,
}: {
  params: { category: string }
}) {
  // Check if the category exists
  if (!Object.keys(watchCategories).includes(params.category)) {
    notFound()
  }

  const category = params.category as WatchCategory
  const categoryData = watchCategories[category]

  // Get episodes for this category
  const episodes = await getEpisodes(category)

  return (
    <div className="container mx-auto px-4 py-8">
      <div className={`${categoryData.color} ${categoryData.textColor} p-8 rounded-lg mb-8`}>
        <Link href="/watch" className="flex items-center mb-4 hover:underline">
          <ArrowLeft className="mr-2 h-4 w-4" />
          Back to all videos
        </Link>
        <h1 className="text-3xl font-bold mb-4">{categoryData.heading}</h1>
        <p className="text-lg">{categoryData.intro}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {episodes.map((episode) => (
          <div
            key={episode.slug}
            className="border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow"
          >
            {episode.thumbnail && (
              <img
                src={episode.thumbnail || "/placeholder.svg"}
                alt={episode.title}
                className="w-full h-48 object-cover"
              />
            )}
            <div className="p-4">
              <h2 className="text-xl font-semibold mb-2">{episode.title}</h2>
              <p className="text-gray-600 mb-4">{episode.synopsis}</p>
              <Link href={`/watch/${episode.slug}`} className="text-blue-600 hover:underline">
                Watch now
              </Link>
            </div>
          </div>
        ))}
      </div>
    </div>
  )
}
