...
export function MegaMenu({ items, className }: MegaMenuProps) {
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent, item: MenuItem, index: number) => {
      switch (e.key) {
        ...
        case " ":
          e.preventDefault()
          if (item.subItems) setActiveMenu(activeMenu === item.label ? null : item.label)
  ...
  // Route-matching logic for active state
  ...
  // Render dropdown menu (for subItems)
        ...
        {item.subItems.map((subItem, idx) => (
          <Link
            ...
            className={cn(
              ...
              focusedIndex === idx && "bg-gradient-to-r from-blue-50 to-cyan-50"
            )}
            onClick={() => {
              setActiveMenu(null)
              setFocusedIndex(-1)
