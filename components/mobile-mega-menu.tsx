    ...
    <div className="bg-gradient-to-b from-gray-50/80 to-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden">
      <nav className={cn("p-4", className)} aria-label="Mobile menu">
        {items.map((item) => {
          ...
          const isLargeSubmenu = item.label === "Blog" || item.label === "Watch" || item.label === "About"

          const renderSubItem = (subItem: NonNullable<MenuItem["subItems"]>[number], idx: number) => (
            <Link
              key={idx}
              href={subItem.href}
              className={cn(
            ...
            >
              ...
              <div className="ml-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:translate-x-1 relative z-10 flex-shrink-0 pointer-events-none">
        ...
        <div className="mt-6 pt-4 border-t border-gray-200/50">
          <Link
            ...
            onClick={onItemClick}
          >
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
