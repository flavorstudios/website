...
- **Suggested fix**: force test mode explicitly (e.g., set `NEXT_PUBLIC_TEST_MODE=1` in the Playwright test environment or gate the Firebase form behind a dedicated `clientEnv.ENABLE_FIREBASE_LOGIN` flag). That keeps the legacy form active in CI, ensuring the alert remains deterministic.
