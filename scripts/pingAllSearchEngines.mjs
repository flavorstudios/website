// scripts/pingAllSearchEngines.mjs
...
/**
 * Load env vars from a file without relying on external deps.
...
function normalizeUrl(urlPath) {
  ...
  if (cleanedUrl.startsWith(`${siteUrl}/`)) {
    const regex = new RegExp(
      `^${siteUrl.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&')}\\/(${siteUrlHostname.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&')}|${siteUrlHostname
        .replace('www.', '')
        .replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&')})`
    );
...
async function getUrlsFromSitemaps() {
  ...
  for (const sitemap of sitemaps) {
    try {
      ...
      const locs =
        parsed.urlset && parsed.urlset.url ? parsed.urlset.url.map((u) => u.loc[0]).filter(Boolean) : [];
